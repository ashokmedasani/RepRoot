> **DEPRECATED — DO NOT TRUST THE "PLANNING ONLY" STATUS BELOW.** Verified against real code on
> 2026-08-03: this design is actually built almost in full (models, `plan_lock_status.py`,
> `plan_lock_cascade.py`, `account_lifecycle.py`, cancellation flow, cold storage, drag-reorder —
> all live). Its §2 audit findings are stale fixes already applied. Real remaining gaps: reorder
> has no freeze gate after downgrade takes effect, no "N clients affected / unassign all" UI for
> a locked shared resource, and `subcategories_per_category` isn't shown in the plan UI. Storage
> numbers here (1GB/5GB) don't match real settings.py (100MB/1GB/10GB) either. This file is
> queued for deletion — see `AGENTS.md` for the rule covering this folder. Do not read this file
> as current product behavior; treat `backend/config/settings.py` and the real source as truth.
