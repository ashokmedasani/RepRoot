...
def lead_submitted(sender, instance, created, **kwargs):
  if created:
    ...
    notify_professional(professional, category='forms', event_type='lead.submitted', event_key=f'lead:{instance.pk}:submitted',
      ...
      action_url=f'/professional/forms-groups/requests/{instance.pk}', payload={'submission_id': instance.pk, 'reference_id': instance.reference_id}, requires_action=True)
...
def lead_meeting_requested(sender, instance, created, **kwargs):
  if created:
    ...
    notify_professional(professional, category='meetings', event_type='lead_meeting.requested', event_key=f'lead-meeting:{instance.pk}:requested',
      ...
      action_url='/professional/forms-groups', payload={'meeting_request_id': instance.pk, 'source': 'lead_form'}, requires_action=True, priority='high')
...
def client_change_requested(sender, instance, created, **kwargs):
  if created:
    ...
    notify_professional(instance.client.professional, category='clients', event_type=f'client.{instance.request_type}.requested', event_key=f'client-change:{instance.pk}:requested',
      title=f'Client {label} request', body=f'{instance.client.username} submitted a {label} request.', action_url=f'/professional/client/{instance.client_id}', payload={'client_id': instance.client_id, 'request_id': instance.pk}, requires_action=True, priority='high')
...
def template_assigned(sender, instance, created, **kwargs):
  if created:
    notify_client(instance.client, category='templates', event_type='template.assigned', event_key=f'assignment:{instance.pk}:created', title='New template assigned', body=f'{instance.template.name} is ready for you.', action_url='/client/templates', payload={'assignment_id': instance.pk, 'template_id': instance.template_id})
...
def tracking_entry_created(sender, instance, created, **kwargs):
  if created and not instance.edited_by_professional:
    notify_professional(instance.client.professional, category='progress', event_type='tracking.entry.created', event_key=f'tracking:{instance.pk}:created', title='Client tracking update', body=f'{instance.client.username} added {instance.template_name}.', action_url=f'/professional/client/{instance.client_id}', payload={'client_id': instance.client_id, 'entry_id': instance.pk})


@receiver(post_save, sender=ProgressEntry)
def progress_created(sender, instance, created, **kwargs):
  if created:
    notify_client(instance.client, category='progress', event_type='progress.created', event_key=f'progress:{instance.pk}:created', title='Progress update added', body=instance.title, action_url='/client/progress', payload={'progress_id': instance.pk})


@receiver(post_save, sender=ClientReminder)
def reminder_created(sender, instance, created, **kwargs):
  if created:
    notify_client(instance.client, category='reminders', event_type='reminder.created', event_key=f'reminder:{instance.pk}:created', title='New reminder', body=instance.title, action_url='/client/meetings', payload={'reminder_id': instance.pk})


@receiver(post_save, sender=ScheduledMeeting)
def meeting_created(sender, instance, created, **kwargs):
  if created:
    notify_client(instance.client, category='meetings', event_type='meeting.scheduled', event_key=f'meeting:{instance.pk}:scheduled', title='Meeting invitation', body=instance.title, action_url='/client/meetings', payload={'meeting_id': instance.pk}, requires_action=True, priority='high')
...
def chat_created(sender, instance, created, **kwargs):
  ...
  if instance.sender == ChatMessage.SENDER_CLIENT:
    notify_professional(instance.professional, action_url=f'/professional/client/{instance.client_id}', **common)
  else:
    notify_client(instance.client, action_url='/client/professional', **common)
