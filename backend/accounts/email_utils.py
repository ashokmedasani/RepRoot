...
def _send_mail(
  ...
  *,
  ...
  """Send one message while supporting the same options as ``send_mail``.

  Django's convenience ``send_mail`` function does not accept ``reply_to``.
  Support and onboarding messages need that header, so build the equivalent
  message explicitly instead of passing an unsupported keyword argument.
  """
  if connection is None and (auth_user is not None or auth_password is not None):
...
def send_mail_background(
  ...
  """Fire-and-forget wrapper around Django email delivery. Returns
